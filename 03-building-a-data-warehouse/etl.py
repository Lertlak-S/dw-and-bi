...
def main(dataset_id, table_id, file_path):
    ...
    keyfile = "../credentials/MyDWProject-swu-ds525-load-data-to-bigquery-413911-1e2437691ee2.json"
    service_account_info = json.load(open(keyfile))
    credentials = service_account.Credentials.from_service_account_info(service_account_info)

    # โค้ดส่วนนี้จะเป็นการสร้าง Client เชื่อมต่อไปยังโปรเจค GCP ของเรา โดยใช้ Credentials ที่
    # สร้างจากโค้ดข้างต้น
    project_id = "articulate-case-413911"
    ...
    if table_id == "actors":
        job_config = bigquery.LoadJobConfig(
            ...
            schema=[
                ...
                bigquery.SchemaField("url", bigquery.SqlTypeNames.STRING)
            ],
        )
    elif table_id == "repo":
        job_config = bigquery.LoadJobConfig(
...
if __name__ == "__main__":
    ...
    file_name_1 = "github_actors.csv"
    file_name_2 = "github_repo.csv"
    file_name_3 = "github_events.csv"

    with open(file_name_1, "w") as csv_file:
    ...
    with open(file_name_2, "w") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([
            "id", 
            "name"
    ...
    with open(file_name_3, "w") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([
            "id", 
            "type",
    ...
    main(dataset_id="github", table_id="actors", file_path=file_name_1)
    main(dataset_id="github", table_id="repo", file_path=file_name_2)
    main(dataset_id="github", table_id="events", file_path=file_name_3)